...
function is_logged_in($userName,$password){
    $data = url_get_contents2('http://172.21.0.1:8080/student/login?studentNumber='.$userName.'&password='.$password);
...
function is_authenticated($studentNumber,$session){
    $data = url_get_contents2('http://172.21.0.1:8080/authenticate?studentNumber='.$studentNumber.'&session='.$session);
    $data = str_replace("}","",$data);
    $data = str_replace("{","",$data);
    $data = str_replace("\"result\":","",$data);
    $data = str_replace("\"","",$data);
